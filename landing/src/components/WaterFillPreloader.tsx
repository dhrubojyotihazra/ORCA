...
export function WaterFillPreloader({
  ...
  // Dynamic telemetry status message based on loading percentage
  const telemetryStage =
    progress < 25
      ? 'Connecting ISRO Telemetry Streams'
      : progress < 50
      ? 'Calibrating Oceansat-3 OCM Grids'
      : progress < 78
      ? 'Evaluating Wave Spectra & HSI'
      : progress < 99
      ? 'Initializing Multi-Agent Mesh'
      : 'System Ready';
      ...
      {!isFinished && (
        ...
        >
          ...
          >
            ...
            {/* ── Progress Counter & Telemetry Status (Woblo Monospace Layout) ── */}
            <div
              className="mt-6 flex flex-col gap-2.5 w-full max-w-[400px]"
              style={{ fontFamily: 'var(--font-mono), monospace' }}
            >
              {/* Telemetry Stage & Percentage */}
              <div className="flex items-center justify-between text-xs sm:text-sm">
                <div className="flex items-center gap-2">
                  <span className="size-2 rounded-full bg-teal-400 animate-pulse shadow-[0_0_8px_#3bf4e4]" />
                  <span className="text-[11px] sm:text-xs font-mono uppercase tracking-[0.18em] text-teal-300/80">
                    {telemetryStage}
                  </span>
                </div>
                <div className="font-mono text-xs sm:text-sm font-bold tracking-widest text-white">
                  {Math.floor(progress)}%
                </div>
              </div>

              {/* Sleek Neon Loading Track Bar */}
              <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden p-[0.5px]">
                <div
                  className="h-full bg-gradient-to-r from-teal-500 via-teal-300 to-cyan-200 rounded-full transition-all duration-75 shadow-[0_0_12px_rgba(59,244,228,0.7)]"
                  style={{ width: `${progress}%` }}
                />
              </div>

              {/* Subtitle / Hint */}
              <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono tracking-wider pt-0.5">
                <span>ORCA v2.6 · SIH26176</span>
