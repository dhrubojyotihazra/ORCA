...
export default function AuthSectionOne({ initialMode = "login" }: AuthSectionOneProps) {
  ...
  const s = {
    ...
    // Google OAuth Button: neomorphic white card in light mode, obsidian glass card in dark mode
    googleBtn: isLight
      ? "bg-[#ffffff] text-[#1e293b] border border-slate-200/90 shadow-[-2px_-2px_6px_rgba(255,255,255,0.95),2px_2px_6px_rgba(180,195,215,0.45)] hover:bg-[#f8fafc] hover:shadow-[-3px_-3px_8px_rgba(255,255,255,1),3px_3px_8px_rgba(180,195,215,0.6)] hover:text-black"
      : "bg-gradient-to-b from-[#111c2a] to-[#0a121c] text-slate-100 border-t border-t-white/20 border-x border-x-white/5 border-b border-b-black/90 shadow-[0_4px_16px_rgba(0,0,0,0.7),inset_0_1px_1px_rgba(255,255,255,0.12)] hover:border-cyan-400/40 hover:shadow-[0_4px_22px_rgba(6,182,212,0.3)] hover:text-white",

    // Divider tokens
    dividerBadge: isLight
      ? "bg-[#e2ebf4] text-slate-500 border border-slate-200/80 shadow-[inset_1px_1px_2px_rgba(180,195,215,0.4)]"
      : "bg-[#050912] text-slate-400 border border-white/5 shadow-[inset_1px_1px_2px_rgba(0,0,0,0.8)]",

    dividerLine: isLight ? "border-slate-300/80" : "border-white/10",
  };
          ...
          >
            ...
            <div className="relative flex items-center justify-center my-3 sm:my-3.5">
              <div className="absolute inset-0 flex items-center">
                <div className={`w-full border-t ${s.dividerLine}`} />
              </div>
              <span className={`relative px-3 py-0.5 text-[10px] sm:text-[11px] font-mono uppercase tracking-wider font-semibold rounded-full ${s.dividerBadge}`}>
                or continue with
              </span>
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className={`w-full min-h-[48px] py-2.5 px-4 rounded-2xl flex items-center justify-center gap-3 text-xs sm:text-sm font-bold transition-all duration-300 cursor-pointer touch-manipulation active:scale-[0.98] ${s.googleBtn}`}
            >
              <svg className="size-4 sm:size-4.5 shrink-0" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
